%% Running the Test with 2 second Windows
...
for track = 1:num_tks
    ...
    % Loads and queries 2 seconds of every mp3 with varying SNR ratios, and
    % the 2 second slides by a second across the entire file.
    [query,fs] = audioread(tks{track});
    query_len = length(query);
    num_wins(track) = floor(query_len/fs)-1;
    
    for j = 1:num_wins(track)
        query_t = query((j-1)*fs+1:(j-1)*fs+1+2*fs,:);
...
save('ShazamData2sec.mat','solution');
...
title('Percentage of Correctly Matched Tracks vs SNR for Two Second Windows')
