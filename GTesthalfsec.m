%% Running the Test with .5 second Windows
...
for track = 1:num_tks
    ...
    % Loads and queries .5 seconds of every mp3 with varying SNR ratios, and
    % the .5 seconds slides by a quarter second across the entire file.
    [query,fs] = audioread(tks{track});
    query_len = length(query);
    num_wins(track) = floor(query_len/fs*4)-1;
    
    for j = 1:num_wins(track)
        query_t = query((j-1)/4*fs+1:(j-1)/4*fs+1+fs/2,:);
...
save('ShazamDatahalfsec.mat','solution');
...
title('Percentage of Correctly Matched Tracks vs SNR for Half Second Windows')
